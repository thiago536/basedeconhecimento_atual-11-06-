    ...
    <>
      ...
      <div className="grid gap-4 py-4 max-h-[60vh] overflow-y-auto pr-4">
        <div className="grid gap-2">
          ...
          <Input id="title" value={formData.title || ''} onChange={(e) => handleChange('title', e.target.value)} placeholder="Título do artigo" />
        </div>
        <div className="grid gap-2">
        ...
        <div className="grid gap-2">
          ...
          <Textarea id="description" value={formData.description || ''} onChange={(e) => handleChange('description', e.target.value)} placeholder="Escreva o conteúdo do artigo aqui..." rows={6} />
        </div>
        <div className="grid gap-2">
    ...
    <main className="flex-1 bg-gray-50/50 dark:bg-gray-900/50 p-6 md:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="mt-2 text-lg text-gray-600 dark:text-gray-400">Encontre artigos, tutoriais e soluções para as suas dúvidas.</p>
        </div>

        <div className="mb-8 flex flex-col items-center gap-4 md:flex-row">
          <div className="relative flex-1 w-full">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
            <Input className="pl-10 h-11" placeholder="Pesquisar artigos..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
          <Select value={selectedCategory} onValueChange={setSelectedCategory}>
            <SelectTrigger className="w-full md:w-[220px] h-11"><SelectValue placeholder="Categorias" /></SelectTrigger>
            <SelectContent>
        ...
        <AlertDialog open={!!faqParaRemover} onOpenChange={(open) => !open && setFaqParaRemover(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>Tem a certeza de que deseja remover o artigo "{faqParaRemover?.title}"?</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        {loading || isPending ? (
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 6 }).map((_, i) => (<Card key={i} className="p-4"><Skeleton className="h-48 w-full rounded-lg" /></Card>))}
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              ...
              filteredFaqs.map((artigo) => (
                <Card key={artigo.id} className="flex flex-col h-full rounded-xl overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 border-gray-200/80 dark:border-gray-800/80">
                  ...
                  <CardFooter className="p-4 pt-4 bg-gray-50 dark:bg-gray-800/50 mt-auto flex justify-between items-center">
                    <Button variant="outline" className="flex-1" onClick={() => router.push(`/base-conhecimento/${artigo.id}`)}>Ver Detalhes</Button>
                    <div className="flex">
